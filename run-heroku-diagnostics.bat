...
call npx heroku run "node backend/src/scripts/check_analytics_data.js" --app energy-audit-store
...
if %ERRORLEVEL% NEQ 0 (
  ...
  echo.
  echo heroku run 'node backend/src/scripts/check_analytics_data.js' --app energy-audit-store
  echo.
)
