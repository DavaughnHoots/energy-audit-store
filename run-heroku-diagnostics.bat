...
call npx heroku run "node backend/scripts/heroku_check_analytics.js" --app energy-audit-store
...
if %ERRORLEVEL% NEQ 0 (
  ...
  echo.
  echo heroku run 'node backend/scripts/heroku_check_analytics.js' --app energy-audit-store
  echo.
)
