...
export class AnalyticsService {
    pool; // Using any to bypass TypeScript errors with pg pool
    constructor(pool) {
        this.pool = pool;
    }
    /**
    ...
    async createOrUpdateSession(sessionId, userId) {
        try {
            const existingSession = await this.pool.query('SELECT * FROM analytics_sessions WHERE id = $1', [sessionId]);
            if (existingSession.rows.length === 0) {
                // Create new session
                await this.pool.query('INSERT INTO analytics_sessions (id, user_id, created_at, updated_at) VALUES ($1, $2, NOW(), NOW())', [sessionId, userId || null]);
            }
            else {
                // Update existing session
                await this.pool.query('UPDATE analytics_sessions SET updated_at = NOW(), user_id = COALESCE($1, user_id) WHERE id = $2', [userId || null, sessionId]);
            }
        }
        catch (error) {
    ...
    async trackEvent(sessionId, eventType, area, data) {
        try {
            ...
            const session = await this.pool.query('SELECT user_id FROM analytics_sessions WHERE id = $1', [sessionId]);
            const userId = session.rows.length > 0 ? session.rows[0].user_id : null;
            // Write event directly to the database
            await this.pool.query('INSERT INTO analytics_events (id, session_id, user_id, event_type, area, data, created_at) VALUES ($1, $2, $3, $4, $5, $6, NOW())', [this.generateUUID(), sessionId, userId, eventType, area, JSON.stringify(data)]);
        }
        catch (error) {
    ...
    async getPlatformMetrics(timeframe) {
        const timeframeClause = this.getTimeframeClause(timeframe);
        try {
            const [userStats, auditStats, productStats, mostVisitedPages, mostUsedFeatures] = await Promise.all([
     ...
     */
    async getMostVisitedPages(timeframeClause) {
        try {
     ...
     */
    async getMostUsedFeatures(timeframeClause) {
        try {
    ...
    async getAuditStats(userId, timeframeClause) {
        try {
         ...
         FROM energy_audits
         WHERE user_id = $1 AND created_at ${timeframeClause}`, [userId]);
            return {
    ...
    async getAggregateAuditStats(timeframeClause) {
        try {
            const result = await this.pool.query(`SELECT COUNT(*) as total_audits
         FROM energy_audits
...
//# sourceMappingURL=AnalyticsService.enhanced.js.map