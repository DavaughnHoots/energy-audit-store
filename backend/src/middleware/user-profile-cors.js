/**
 * Special CORS middleware specifically for user profile routes
 * This ensures CORS headers are set before any auth checks for user profile routes
 */

import { appLogger } from '../utils/logger.js';

export function userProfileCorsMiddleware(req, res, next) {
  try {
    // Log every request to user profile endpoints for debugging
    appLogger.info('User profile CORS middleware executing', {
      path: req.path,
      method: req.method,
      origin: req.headers.origin,
      isUserProfileRoute: req.path.includes('/profile'),
      requestId: req.id || 'no-request-id'
    });

    // Set CORS headers for user profile routes
    ...
    appLogger.info('User profile CORS processing origin', {
    ...
    appLogger.error('Error in user profile CORS middleware', {
      error: error.message,
      stack: error.stack,
      path: req.path,
      method: req.method
