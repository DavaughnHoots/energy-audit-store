// Admin analytics with embedded mock data fix
declare global {
  namespace Express {
    interface Request {
      id: string;
...
// Import fixed admin analytics routes with embedded mock data
...
// Register admin analytics routes under /api/admin/analytics path - USING FIXED VERSION WITH EMBEDDED MOCK DATA
...
app.get('/api/debug/config', (req: Request, res: Response) => {
  res.json({ 
    message: 'Using fixed version with embedded mock data',
    timestamp: new Date().toISOString(),
    routes: {
      admin: 'Enhanced',
      analytics: 'Standard with enhanced service',
      adminAnalytics: 'FIXED VERSION WITH EMBEDDED MOCK DATA',
...
const server = app.listen(PORT, () => {
  appLogger.info('Server started with embedded mock data fix', createLogMetadata(undefined, {
    port: PORT,
    nodeEnv: process.env.NODE_ENV,
    version: 'admin-analytics-embedded-mock-data-fix-v1'
  }));
});
