...
interface DashboardStats {
  ...
  energyAnalysis?: {
    ...
    savingsAnalysis: { name: string; estimatedSavings: number; actualSavings: number }[];
  };
...
const NewUserDashboardPage: React.FC = () => {
  ...
  const [stats, setStats] = useLocalStorage<DashboardStats>('dashboard-stats-v2', {
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      const totalEstimated = reportData.recommendations?.reduce((sum, rec) => {
        let savings = 0;
        if (typeof rec.estimatedSavings === 'number') {
          savings = rec.estimatedSavings;
        } else if (rec.estimatedSavings) {
          const parsed = parseFloat(String(rec.estimatedSavings).replace(/[^0-9.-]+/g, '') || '0');
      ...
      const totalActual = reportData.recommendations?.reduce((sum, rec) => {
        let savings = 0;
        if (typeof rec.actualSavings === 'number') {
          savings = rec.actualSavings;
        } else if (rec.actualSavings) {
          const parsed = parseFloat(String(rec.actualSavings).replace(/[^0-9.-]+/g, '') || '0');
          savings = isNaN(parsed) ? 0 : parsed;
        }
        return sum + savings;
      }, 0) || 0;
