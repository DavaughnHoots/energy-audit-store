...
import { useUserBadges, useRecentAchievements } from '../hooks/useBadgeProgress';
...
const BadgesDiagnosticPage: React.FC = () => {
  ...
  const { achievements, loading: achievementsLoading } = useRecentAchievements(5);
  ...
  async function handleForceEvaluation() {
    ...
    try {
      ...
      setEvaluationError(`Error: ${err instanceof Error ? err.message : String(err)}`);
    } finally {
      setRefreshing(false);
    }
  }
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      <div className="flex flex-col gap-8">
        <div className="flex flex-row justify-between items-center">
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              className="flex items-center gap-2"
            >
              {refreshing ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />}
              Force Evaluation
            </Button>
            <Button
          ...
          {loading ? (
            ...
            <div className="flex flex-col gap-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p><strong>Total Badge Types:</strong> {allBadges.length}</p>
                  <p><strong>Earned Badges:</strong> {earnedBadges.length}</p>
                  <p><strong>In-Progress Badges:</strong> {inProgressBadges.length}</p>
                  <p><strong>Locked Badges:</strong> {lockedBadges.length}</p>
                </div>
                <div>
                  <p><strong>Points:</strong> {points?.points || 0}</p>
                  <p><strong>Level:</strong> {points?.level || 1}</p>
                  <p><strong>Current Title:</strong> {points?.title || 'N/A'}</p>
                  <p><strong>Next Level Points:</strong> {points?.nextLevelPoints || 100}</p>
                </div>
              </div>
          ...
          {achievementsLoading ? (
            ...
            <div className="grid grid-cols-1 gap-4">
              {achievements.map((achievement) => (
                <div key={achievement.id} className="p-3 bg-white rounded-md border">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {achievement.reward && (
                        <p className="text-xs text-green-600 mt-1">{achievement.reward.description}</p>
                      )}
                    </div>
                  </div>
                </div>
        ...
        <div className="p-4 bg-gray-50 rounded-lg">
          <h2 className="text-lg font-semibold mb-2">Badge Data (Debug View)</h2>
          
          <div className="grid grid-cols-1 gap-4">
            <div>
              <h3 className="font-medium mb-1">Earned Badges ({earnedBadges.length})</h3>
              {earnedBadges.length > 0 ? (
                <div className="divide-y bg-white rounded-md border overflow-hidden">
                  {earnedBadges.map(badge => (
                    <div key={badge.badgeId} className="p-3 flex justify-between items-center">
                      <div>
                        <span className="font-medium">{badge.badgeId}</span>
                        <div className="text-xs text-gray-500">
                          {badge.earnedDate && (
                            <span>Earned at: {new Date(badge.earnedDate).toLocaleString()}</span>
                          )}
                        </div>
                      </div>
            ...
            <div>
              <h3 className="font-medium mb-1">In-Progress Badges ({inProgressBadges.length})</h3>
                ...
                <div className="divide-y bg-white rounded-md border overflow-hidden">
                  {inProgressBadges.map(badge => (
                    <div key={badge.badgeId} className="p-3 flex justify-between items-center">
                      ...
                      <div className="text-blue-600 font-semibold">{badge.progress}%</div>
                    </div>
                  ))}
                </div>
              ) : (
            ...
            <div>
              <h3 className="font-medium mb-1">Badge Definitions ({allBadges.length})</h3>
              <div className="max-h-60 overflow-y-auto divide-y bg-white rounded-md border">
                {allBadges.map(badge => (
                  <div key={badge.id} className="p-3 flex justify-between items-center">
                    <div>
                      <span className="font-medium">{badge.name}</span>
                      <div className="text-xs text-gray-500">
                        ID: {badge.id} | Category: {badge.category} | Tier: {badge.tier}
                      </div>
                    </div>
