...
// See mobile-home-defaults.txt for detailed documentation and sources

// Helper function to determine construction period from year built
export const getConstructionPeriod = (yearBuilt: number): string => {
...
export const getSizeCategory = (squareFootage: number): 'small' | 'medium' | 'large' => {
  if (squareFootage < 1000) return 'small';
  if (squareFootage <= 1800) return 'medium';
  return 'large';
};

// Climate zone mappings by state
...
// Helper function to get climate zone
...
export const climateZoneAdjustments: Record<number, number> = {
  1: 0.85, // very hot
  2: 0.9,  // hot
  3: 0.95, // warm
  4: 1.0,  // mixed
...
export const energyUsageBaseline: Record<string, number> = {
  'pre-1976': 70,
  '1976-1994': 65,
  '1994-2000': 58,
  'post-2000': 52
...
export const getMobileHomeDefaults = (yearBuilt: number, squareFootage: number, state?: string) => {
  const constructionPeriod = getConstructionPeriod(yearBuilt);
  const sizeCategory = getSizeCategory(squareFootage);
  ...
  if (state) {
    ...
    const climateAdjustment = climateZoneAdjustments[climateZone] || 1.0;
    const sizeAdjustment = sizeAdjustments[sizeCategory] || 1.0;
    const baseEnergyUsage = energyUsageBaseline[constructionPeriod] || 58; // Default to average if not found
