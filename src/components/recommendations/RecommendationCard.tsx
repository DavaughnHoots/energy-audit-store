  ...
  const renderStatusEdit = () => {
    ...
    const [actualSavings, setActualSavings] = React.useState<string>(
      recommendation.actualSavings
        ? recommendation.actualSavings.toString()
        : "",
    );
  ...
  const renderImplementationDetailsEdit = () => {
    ...
    const [implementationCost, setImplementationCost] = React.useState<string>(
      recommendation.implementationCost
        ? recommendation.implementationCost.toString()
        : "",
    );
            ...
            <ul className="space-y-2">
              {(editHistory?.[recommendation.id] || []).map((edit, index) => (
                <li key={index} className="text-xs text-gray-600">
                  ...
                  <span className="font-medium">{edit.oldValue || "none"}</span>{" "}
                  to <span className="font-medium">{edit.newValue}</span>
                </li>
              ))}
            </ul>
          </div>
    ...
    if (displayMode === "compact") {
      return (
        <div className="flex items-center">
          ...
          <span
            className={`font-medium capitalize ${recommendation.status === "implemented" ? "text-green-600" : "text-blue-600"}`}
          >
          ...
          <div className="flex items-center">
            ...
            <select
              ...
              onChange={(e) =>
                onUpdateStatus(
                  recommendation.id,
                  e.target.value as "active" | "implemented",
                )
              }
              disabled={isLoading}
            >
      ...
      <div className="mt-6 relative border rounded-lg p-4">
        ...
        {productSuggestions.length > 0 ? (
          ...
          <div className="bg-gray-100 p-3 rounded-lg text-gray-600 text-sm">
            <p className="mb-2">
              No specific product suggestions available for this recommendation.
            </p>
            ...
            <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              ...
              <p className="text-xs text-blue-700 mb-2">
                We suggest exploring energy-efficient products in the{" "}
      ...
      <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gray-50 p-3 rounded-lg">
          ...
          <div className="flex items-center">
            <span className="font-medium text-green-600">
              {formatCurrency(recommendation.estimatedSavings)}/year
            </span>
        ...
        {displayMode !== "compact" &&
          recommendation.status === "implemented" && (
            <div
              className={`${isEditingImplementation ? "col-span-1 md:col-span-3" : ""} bg-gray-50 p-3 rounded-lg`}
            >
              <span className="text-sm text-gray-500 flex items-center">
                <DollarSign className="h-4 w-4 mr-1 text-green-500" />
                Actual Savings:
              ...
              {isEditingSavings ? (
                ...
                <div className="flex items-center">
                  <span className="font-medium text-green-600">
                    {recommendation.actualSavings
                      ? formatCurrency(recommendation.actualSavings) + "/year"
                      : "Not reported"}
                  </span>
