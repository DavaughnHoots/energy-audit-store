...
import { ProductSuggestionCardProps } from './types';
 ...
 */
const getEnergyStarUrl = (product: any): string => {
 ...
 */
const getAmazonUrl = (product: any): string => {
 ...
 */
const getHomeDepotUrl = (product: any): string => {
  const searchTerm = `energy efficient ${product.name}`;
  return `https://www.homedepot.com/s/${encodeURIComponent(searchTerm)}?NCNI-5`;
};

/**
 * ProductSuggestionCard component
 * Displays a product recommendation card with image, details, and shopping links
 */
