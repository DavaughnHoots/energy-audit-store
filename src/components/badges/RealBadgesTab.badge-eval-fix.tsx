...
import { useUserBadges } from '../../hooks/useBadgeProgress.badge-fix';
...
/**
 * Enhanced BadgesTab component with improved badge classification logic
 * Uses the badge-fix hook to properly categorize badges based on actual progress
 */
const RealBadgesTabBadgeEvalFix: React.FC = () => {
  ...
  // Use the enhanced badge hook with badge evaluation fixes
  const { 
    loading, 
    error, 
    allBadges, 
...
function prepareBadgesForDisplayWithNormalization(allBadges: any[] | null | undefined, userBadges: any[] | null | undefined): any[] {
  ...
  if (badgeDefinitions.length === 0 && userBadges.length > 0) {
    console.warn('Failed to map any user badges to definitions with normalization', { 
      userBadgesCount: userBadges.length,
      allBadgesCount: allBadges.length,
      sampleUserBadge: userBadges[0],
      sampleAllBadge: allBadges[0],
      firstFewUserBadgeIds: userBadges.slice(0, 3).map(ub => ub.badgeId || ub.id),
      firstFewBadgeDefinitionIds: allBadges.slice(0, 5).map(bd => bd.id)
    });
  } else if (badgeDefinitions.length > 0) {
    console.log(`✅ Successfully mapped ${badgeDefinitions.length} badges out of ${userBadges.length} user badges`);
  }
  
  return badgeDefinitions;
}

export default RealBadgesTabBadgeEvalFix;