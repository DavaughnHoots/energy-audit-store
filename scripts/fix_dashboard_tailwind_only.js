// scripts/fix_dashboard_tailwind_only.js

const fs = require('fs');
const path = require('path');

/**
 * Fix dashboard access by replacing Material UI components with Tailwind-only components
 * This ensures that the dashboard is accessible from all devices including iOS
 */
async function applyDashboardFix() {
  try {
    console.log('Starting dashboard Tailwind-only fix...');
    
    // Apply the authentication fix first
    console.log('Applying authentication fix...');
    require('./fix_mobile_auth_unified');
    
    // Path to RoadmapFeature.tsx
    const roadmapFeaturePath = path.resolve('src/components/admin/RoadmapFeature.tsx');
    console.log(`Updating file: ${roadmapFeaturePath}`);
    
    // Replace the entire file with a Tailwind-only version (no Material UI dependencies)
    const tailwindOnlyContent = `import React, { useState, useEffect } from 'react';
          ...
          <div className="space-y-4">
            {roadmapItems.length === 0 ? (
              ...
              roadmapItems.map(item => (
                <div key={item.id} className={\`bg-white p-4 shadow rounded-lg border-l-4 relative hover:shadow-md transition-shadow duration-200
...
export default RoadmapFeature;`;
    ...
    fs.writeFileSync(roadmapFeaturePath, tailwindOnlyContent);
    console.log('✅ Replaced RoadmapFeature.tsx with Tailwind-only version (no Material UI dependencies)');
    
    console.log('✅ Dashboard Tailwind-only fix applied successfully');
    return true;
  } catch (error) {
    console.error('Error applying dashboard Tailwind-only fix:', error);
    return false;
  }
}
...
applyDashboardFix()
  .then(success => {
    console.log(success ? 'Dashboard fix completed successfully' : 'Dashboard fix failed');
