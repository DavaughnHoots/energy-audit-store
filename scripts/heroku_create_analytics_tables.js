/**
 * Heroku-compatible script to check and create analytics tables
 * 
 * This script will connect to the Heroku database, check if the analytics tables exist,
...
// Get database connection from Heroku's DATABASE_URL environment variable
const pool = new Pool({
...
async function checkTablesExist() {
  ...
  try {
    console.log('Connected to Heroku database, checking if analytics tables exist...');
...
async function createAnalyticsTables() {
  const client = await pool.connect();
  try {
    console.log('Creating analytics tables on Heroku...');
    ...
    console.log('All tables created successfully on Heroku.');
    return true;
  } catch (error) {
...
async function main() {
  try {
    ...
    if (!eventsTableExists || !sessionsTableExists || !consentTableExists) {
      console.log('Some analytics tables are missing. Creating all tables on Heroku...');
      await createAnalyticsTables();
      console.log('Tables created successfully. Analytics should now work properly.');
    } else {
      console.log('All analytics tables exist on Heroku. No action required.');
    }
    ...
    if (tablesAfter.eventsTableExists && tablesAfter.sessionsTableExists && tablesAfter.consentTableExists) {
      console.log('SUCCESS: All required tables are now available in the Heroku database.');
    } else {
      console.error('ERROR: Table creation failed. Some tables are still missing on Heroku.');
    }
  } catch (error) {
    console.error('Error in main execution:', error);
  } finally {
    await pool.end();
    console.log('Heroku database connection closed.');
  }
}
